...
import { OktaAuthHttpInterface, RequestOptions, FetchOptions, RequestData, HttpResponse } from './types';
...
const parseInsufficientAuthenticationError = (header: string): InsufficientAuthenticationError => {
	if (!header) {
...
export function httpRequest<T = any>(sdk: OktaAuthHttpInterface, options: RequestOptions): Promise<T> {
	options = options || {};
