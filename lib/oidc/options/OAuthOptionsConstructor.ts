...
function assertValidConfig(args) {
	...
	if (!issuer) {
		throw new AuthSdkError(
			'No issuer passed to constructor. ' +
	...
	if (issuer.indexOf('-admin.') !== -1) {
		throw new AuthSdkError(
			'Issuer URL passed to constructor contains "-admin" in subdomain. ' +
...
export function createOAuthOptionsConstructor() {
	...
	return class OAuthOptionsConstructor extends HttpOptionsConstructor implements Required<OktaAuthOAuthOptions> {
		// CustomUrls
		issuer: string;
		authorizeUrl: string;
		userinfoUrl: string;
		...
		constructor(options: any) {
			...
			this.pushAuthorizationUrl = removeTrailingSlash(options?.pushAuthorizationUrl);
			...
			this.provider = options.provider || 'okta-wic';
